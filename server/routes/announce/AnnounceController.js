...
router.put('/:id', /* VerifyToken, */ function (req, res) {
    Announce.findByIdAndUpdate(req.params.id, req.body, {new: true}, function (err, announce) {
        if (err) return res.status(500).send("There was a problem updating the announce.");
        res.status(200).send(announce);
    });
});
